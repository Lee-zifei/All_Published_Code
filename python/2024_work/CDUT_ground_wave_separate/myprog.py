...
def myimtocol2(input1,rn1,rn2,n1,n2, n3,tslide,xslide,f):
    if f == 1:
        ...
        for i in range (n1):
            temp = np.zeros((n2,n3), dtype='float32')
            temp = input1[:,i*n3:(i+1)*n3,]
            if i == 0:
        ...
        num2 = int(np.floor((n3 - rn2) / xslide) + 1 + (np.mod(n3 - rn2, xslide) != 0))
        for i in range (n1):
            temp = np.zeros((num1*num2, n2,n3), dtype='float32')
...
def mycoltoim_torch(input,config):
    input = input.squeeze()
    rn1, rn2, n1, n2, tslide, xslide = config.DATA.IMG_SIZE, config.DATA.IMG_SIZE, config.ROW, config.COL, config.SROW, config.SCOL
    datasize, rn1, rn2 = input.shape
    num1 = int(np.floor((n1 - rn1) / tslide) + 1 + (np.mod(n1 - rn1, tslide) != 0))
    num2 = int(np.floor((n2 - rn2) / xslide) + 1 + (np.mod(n2 - rn2, xslide) != 0))
    output1 = torch.zeros(n1, n2, dtype=torch.float32)
    weight = torch.zeros(n1, n2, dtype=torch.float32)
    one = torch.ones(rn1, rn2, dtype=torch.float32)
    for i in range(num2):
        for j in range(num1):
            if (i < num2 - 1):
                if (j < num1 - 1):
                    output1[j * tslide:j * tslide + rn1, i * xslide:i * xslide + rn2] += input[i * num1 + j, :, :];
                    weight[j * tslide:j * tslide + rn1, i * xslide:i * xslide + rn2] += one;
                else:
                    output1[n1 - rn1:n1, i * xslide:i * xslide + rn2] += input[i * num1 + j, :, :];
                    weight[n1 - rn1:n1, i * xslide:i * xslide + rn2] += one;
            else:
                if (j < num1 - 1):
                    output1[j * tslide:j * tslide + rn1, n2 - rn2:n2] += input[i * num1 + j, :, :];
                    weight[j * tslide:j * tslide + rn1, n2 - rn2:n2] += one;
                else:
                    output1[n1 - rn1:n1, n2 - rn2:n2] += input[i * num1 + j, :, :];
                    weight[n1 - rn1:n1, n2 - rn2:n2] += one;
    return output1 / weight
...
def turnone(inputs):####数据归一化
    n=inputs.shape[0]
    outputs=np.zeros((n,), dtype='float32')
    vmax=max(abs(inputs))
    vmin=min(abs(inputs))
    for i in range(n):
        outputs[i]=(inputs[i]-vmin)/(vmax-vmin)
    return outputs
