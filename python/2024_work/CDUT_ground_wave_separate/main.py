...
def parse_option():
    parser = argparse.ArgumentParser('Deblending Transformer training and evaluation script', add_help=False)
...
def main(config, fine_tune=None):
    dataset_train, dataset_val, data_loader_train, data_loader_val = build_loader(config)

    # print(['length is:'+str(len(data_loader_train))])
    logger.info(f"Creating model:{config.MODEL.TYPE}/{config.MODEL.NAME}")
    model = build_model(config)
    model.cuda()
    if fine_tune:
        checkpoints = torch.load("ckpt_epoch_121.pth", map_location='cpu')
        checkpoint_dict = checkpoints['model']
...
if __name__ == '__main__':
    ...
    fine_tune = config.DATA.FINE_TUNE
    main(config, fine_tune)
