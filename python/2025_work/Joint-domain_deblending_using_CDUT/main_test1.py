...
if __name__ == '__main__':
    ...
    datapath = './test/PGS_blended_data_time/temp'
        ...
        if domain == 'CRG':
            ...
            for i in range(n2):
                ...
                if epoch==0:
                    ...
                    d1_new = np.load('./d1_new_3d.npy')[:,i,:]
                    d2_new = np.load('./d2_new_3d.npy')[:,i,:]
                ...
                for iter in range(niter+1):
                    ...
                    d1de1,d2de2 = iter_in_crg(config,temp_input/max1,net_model,SROW//4,SCOL//4)
                    ...
                    if trace == 55:
                        snr_cpkg = snr1
                        npy2bin(f'snr_{str(epoch)}_{str(niter)}.dat',snr_cpkg)
            ...
            for i in range(n3):
                ...
                for iter in range(niter+1):
                    ...
                    if epoch==0:
                        ...
                        d1_blend = np.load('./d1_new_3d.npy')[:,:,i]
                        d2_blend = np.load('./d2_new_3d.npy')[:,:,i]
                    ...
                    if epoch == 0:
                        max1 = np.max(np.abs(d1_blend))/1.8
                    else:
                        max1 =1
                    ...
                    d1de11,d2de21 = iter_in_csg(config,temp_input/max1,net_model,SROW//4,SCOL//4)
                    
                    temp_input11[0,:,:] = d1de11
                    temp_input11[1,:,:] = d2de21
                    if trace == 55:
                        snr_cpkg = snr(d1de11*max1, d1)
                        npy2bin(f'snr_{str(epoch)}_{str(niter)}.dat',snr_cpkg)
                    
                    d1de1,d2de2 = iter_in_csg(config,temp_input11,net_model+'_temp',SROW//4,SCOL//4)
