...
def patching_2dir(config, inputs, name, targets=None, test=False, denoise=False):
    inputs = inputs.numpy()
    flag1 = flag2 = False 
    index = torch.zeros((2,), dtype=torch.float32)
    names = name.split('.')[0]
...
def patching_2c(config, inputs, name, targets=None, test=False, denoise=False):
    inputs = inputs.numpy()
    flag1 = flag2 = False 
    index = torch.zeros((2,), dtype=torch.float32)
    names = name.split('.')[0]
...
def patching_128img(config, inputs, name, targets=None, test=False, denoise=False):
    inputs = inputs.numpy()
    flag1 = flag2 = False 
    index = torch.zeros((2,), dtype=torch.float32)
    names = name.split('.')[0]
    ...
    if names[0] == 'p':
        ...
        index[0] = 6 if test else 1
        H, W, sh, sw = 725, 128, 32, 10
    elif names[0] == 'l':
        index[0] = 7 if test else 2
        H, W, sh, sw = 900, 128, 40, 10
    elif names[0] == 'a':
        index[0] = 8 if test else 3
        H, W, sh, sw = 1200, 128, 56, 10
        flag1 = True
    elif names[0] == 'e':
        index[0] = 9 if test else 4
        H, W, sh, sw = 1200, 128, 27, 10
