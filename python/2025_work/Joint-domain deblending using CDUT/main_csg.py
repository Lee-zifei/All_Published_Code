...
from subfunctions import myimtocol,read_d2,seis,npy2bin,calculate_snr as snr
import torch
...
def main(config,filed,net_mod,SROW,SCOL):
	...
	checkpoint_dict = torch.load('./output/'+net_mod+'/default/ckpt_epoch_199.pth', map_location='cpu')['model']
...
# def test_a(seeds):
# 	sample = np.load('data/NewData3_csg/Test/target/a1.npy').astype(np.float32)
# 	hyper = np.load('data/NewData3_csg/Test/sample/a1.npy').astype(np.float32)
# 	n1,n2=hyper.shape
# 	hyper = turnone(hyper)
# 	np.random.seed(seeds)
# 	# hyper2 = np.flip(hyper, axis=1)
...
if __name__ == '__main__':
	...
	# filename = './test/nanjing_data_old/temp/deblend1_in_csg.dat'
	# filename = './test/nanjing/temp/crg_deblend1.dat'
	filename_y1 =datapath+'/../blend_data/d1.dat'
	filename = datapath+'/../blend_data/d1b.dat'

	dom = ['CSG']
	net_mod = ['CDUTnet_2single_csg_0.4',
				'CDUTnet_2single_csg_0.4_temp']
	for ii in range(1):
		i = ii
	
		bl1 = read_d2(filename,[n1,n2,n3],i)
		d1_2d = read_d2(filename_y1,[n1,n2,n3],i)
		# max1 = np.max(np.abs(bl1))/4.5
		max1 = 1
		print(max1)
		indata = bl1/max1
		d1de1 = main(config,indata,net_mod[0],SROW,SCOL)
		
		outdata2 = main(config,d1de1,net_mod[1],SROW,SCOL)*max1
  
		snr1 =  snr(outdata2, d1_2d)

		# npy2bin(f'./test/nanjing_data_old/temp/csg1_hyper/deblend_{i+1}.dat',outdata)
		logger.info(f"SNR1={'%.2f'%snr1}")
		# bl1 = bin2npy(filename,n1,n2,i)
		# d1_2d = bin2npy(filename_y1,n1,n2,i)
		# max1 = np.max(bl1)
		# d1de1 = main(config,bl1/max1,SROW,SCOL)
		# npy2bin(f'./test/songtao_test/deblend/deblend2_{i+1}.dat',d1de1*max1)

	# bl1 = np.load(filename)
	# d1 = np.load(filename_hyper)

	# max1 = np.max(bl1)



	# d1de2 = main(config,d1de1,SROW,SCOL)
	...
	ax3.imshow(d1_2d,cmap=mm,vmax = clip,vmin = -clip,aspect=asp)


	ax3 = fig.add_subplot(152)
	ax3.imshow(bl1,cmap=mm,vmax = clip,vmin = -clip,aspect=asp)

	ax5 = fig.add_subplot(153)
	ax5.imshow(outdata2,cmap=mm,vmax = clip,vmin = -clip,aspect=asp)

	ax6 = fig.add_subplot(154)
	ax6.imshow(bl1-outdata2,cmap=mm,vmax = clip,vmin = -clip,aspect=asp)

	ax6 = fig.add_subplot(155)
	ax6.imshow(d1_2d-outdata2,cmap=mm,vmax = clip,vmin = -clip,aspect=asp)
