...
if __name__ == '__main__':
	...
	file1_t = datapath+'/../blend_data/net1_csg.dat'
	file2_t = datapath+'/../blend_data/net2_csg.dat'
	...
	net_mod = ['CDUTnet_2single_crg_1_iter',
					'CDUTnet_2single_crg_1_iter_5',
					'CDUTnet_2single_crg_1_iter_5']
	# dom = ['CRG']
	# net_mod = ['CDUTnet_2single_crg_1_iter_5']
 
	SROW = 48
	SCOL = 48

	# for trace in range(n2):
	for trace in range(60,61,1):
		for epoch in range(1):
			...
			d1_new = read_d3(file1_t,[n1,n2,n3],trace)
			d2_new = read_d3(file2_t,[n1,n2,n3],trace)
			...
			for iter in range(niter+1):
				...
				logger.info(f"正在进行第{str(iter+1+5)}次迭代，当前处理到{domain}第{str(trace+1)}/{str(n2)}道,SNR1={'%.2f'%snr1},SNR2={'%.2f'%snr2}")
			...
			# 	d1de1,d2de2 = iter_in_crg(config,temp_input/max1,net_mod[2],SROW,SCOL)
			...
			# 	logger.info(f"正在进行第{str(iter+1+10)}次迭代，当前处理到{domain}第{str(trace+1)}/{str(n2)}道,SNR1={'%.2f'%snr1},SNR2={'%.2f'%snr2}")
   
			
			d1_newww = (d1_blend-estimate_interference(d2_neww,t2,t1,dt))
			d2_newww = (d2_blend-estimate_interference(d1_neww,t1,t2,dt))
			deblend1[:,trace,:] = d1_newww
			deblend2[:,trace,:] = d2_newww
	np.save('./d1_new_3d_est_crg_only1.npy',deblend1)
	np.save('./d2_new_3d_est_crg_only1.npy',deblend2)
	...
	ax5.imshow(d1_newww,cmap=mm,vmax = clip,vmin = -clip,aspect=asp)

	ax6 = fig.add_subplot(244)
	ax6.imshow(d1-d1_newww,cmap=mm,vmax = clip,vmin = -clip,aspect=asp)
